...
- 首次启动使用可取消、generation 隔离的异步任务生成每台 Mac 独立的 TLS 配对身份；
  PKCS#12 密码保存到 Keychain，旧版密码文件验证后自动迁移并删除。
- 身份生成增加 15 秒超时、原子发布和残留清理；身份不完整或损坏时不再静默替换，用户
  确认重置后才生成新证书，并明确提示 HarmonyOS 设备需要重新配对。
- 创建虚拟显示器后先解除可能存在的镜像关系并等待显示模式稳定，再交给
  ScreenCaptureKit 枚举和采集，降低首次连接出现镜像或空白画面的风险。
